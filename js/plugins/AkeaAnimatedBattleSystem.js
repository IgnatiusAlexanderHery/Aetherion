...
Sprite_Battler.prototype.manageAkeaActions = function (action) {
  ...
  if (action.getTargets() && (!action.getTargets()[0] || action.getTargets()[0].hp == 0)) {
    if (action.getTargets()[0]) action.getTargets()[0].clearAkeaAnimatedBSActions();
  ...
  switch (action.getActionType()) {
    ...
    case "HitAll":
      ...
      BattleManager.akeaEmptyWindow(originalLength);
      for (let t of targets) {
        BattleManager._logWindow.displayChangedStates(t);
      }
      this._battler.getAkeaAnimatedBSActions().originalTarget(this._battler.initialTargets);
      break;
    case "FinishAction":
...
Sprite_Battler.prototype.startMove = function (x, y, duration, jumpHeight = 0, levitation = false) {
  ...
  this._currentJumpHeight = 0;
  if (this._shadowSprite) this._shadowSprite.opacity = levitation ? 0 : 255;
...
Sprite_Battler.prototype.updatePosition = function () {
  ...
  if (this._shadowSprite) this._shadowSprite.y = parseInt(this.getJumpHeight() - 2);
...
Sprite_Battler.prototype.configureAkeaBattlerPoses = function (battler) {
  ...
  if (poses) {
    this.configakeaAnimatedBSPoses(JSON.parse(JSON.parse(poses).poses));
  }
};
Sprite_Battler.prototype.configakeaAnimatedBSPoses = function (posesIndex) {
  ...
  for (const poseIndex of posesIndex) {
    this.setSpecificAkeaPose(JSON.parse(allPoses[poseIndex - 1]));
  }
};
let test;
...
Sprite_Battler.prototype.setAkeaBaseMotion = function () {
  for (let motion in this.akeaMotions) {
    (this.akeaMotions[motion].speed = 12), (this.akeaMotions[motion].inOut = false);
  }
};
...
Sprite_Battler.prototype.configureAkeaBattlerSheet = function (battler) {
  ...
  if (characterSheet) {
    this.setCharacterNewSheet(parseInt(JSON.parse(characterSheet).baseSpritesheet));
  }
};
...
Sprite_Actor.prototype.stepForward = function () {
  ...
  let action = this._battler.getAkeaAnimatedBSActions().unloadAction();
  if (this._offsetX == action.getOffsetX() && this._offsetY == action.getOffsetY()) {
    return;
  }
...
Sprite_Actor.prototype.stepBack = function () {
  if (this._battler._akeaRetreating) {
    return;
  }
...
Sprite_Enemy.prototype.loadBitmap = function (name) {
  ...
  if (nameSv != "") this._mainSprite.bitmap = ImageManager.loadSvEnemy(nameSv);
  else this._mainSprite.bitmap = ImageManager.loadSvEnemy(name);
...
Sprite_Enemy.prototype.updateBitmap = function () {
  ...
  if (needsToUpdateMainSprite) this._mainSprite.setHue(hue);
};
